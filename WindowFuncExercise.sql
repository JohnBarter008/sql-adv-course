...
SELECT
	...
	, cc.Country
	, cc.DailyCases
	, AVG(cc.DailyCases) OVER (PARTITION BY cc.Country ORDER BY cc.DateRecorded ROWS BETWEEN 6 PRECEDING AND CURRENT ROW) / 7.0 AS SevenDayMovingAverageCases
FROM
    CovidCase cc
ORDER BY
	cc.Country
...
ORDER BY Ranking, cc.Country

-- when tied cases appear you might not get the same number of lines for each country.
...
,cc.DateRecorded
,cc.DailyCases
,RANK() OVER (PARTITION BY cc.Country ORDER BY cc.DailyCases DESC)
...
select a.continent, a.iata_code, a.latitude_deg from airports a